...
Application factory and HTTP routes; shared logic is in evidence_constants,
evidence_helpers, and evidence_jobs.
"""
...
# `python src/api/evidence_api.py` and gunicorn `src.api.evidence_api:app`
...
from src.api.evidence_helpers import (
    _debug_ignored,
    _find_company_retained_result,
    _list_quarter_evidence_screenshots,
    _load_retained_earnings_results,
    _playwright_busy_response,
    _run_python_script,
...
def create_app():  # NOSONAR
    ...
    def get_retained_earnings_flow_csv():
        ...
        try:
            ...
            response.headers["Content-Disposition"] = (
                "attachment; filename=retained_earnings_flow.csv"
            )
            # Prevent caching
    ...
    def refresh_data():
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Ownership update timed out; continuing with previous ownership data"
                )
            except subprocess.CalledProcessError as e:
                logger.warning(
                    f"Ownership update failed; continuing with previous data: {e.stderr}"
                )
            except Exception as e:
    ...
    def run_pdfs_pipeline():
        ...
        try:
            ...
            extractor = (
                project_root / "src/extractors/extract_retained_earnings_all_pdfs.py"
            )
            if not downloader.exists() or not extractor.exists():
                return jsonify(
                    {"status": "error", "message": "Pipeline scripts not found"}
                ), 404
            ...
            threading.Thread(
                ...
                args=(project_root, downloader, extractor),
                daemon=True,
            ).start()
            return jsonify(
                {"status": "accepted", "message": "PDF pipeline started in background"}
            ), 202
        except Exception as e:
    ...
    def run_net_profit_scrape():
        ...
        try:
            ...
            if not scraper.exists():
                return jsonify(
                    {"status": "error", "message": "Net profit scraper not found"}
                ), 404
            ...
            return jsonify(
                {
                    "status": "accepted",
                    "message": "Net profit scraping started in background",
                }
            ), 202
        except Exception as e:
        ...
        try:
            ...
            def _finalize_after_stop():
                try:
                    ...
                    logger.info(
                        "[Pipeline] Stop requested: running extractor on downloaded PDFs..."
                    )
                    env = os.environ.copy()
                    ...
                    try:
                        ...
                        if partial.exists():
                            ...
                            logger.info(
                                "[Pipeline] Promoted partial results to retained_earnings_results.json"
                            )
                    except Exception as e:
                        logger.warning(
                            f"[Pipeline] Could not promote partial results: {e}"
                        )
    ...
    def correct_retained_earnings():
        ...
        try:
            subprocess.run(
                [sys.executable, str(PROJECT_ROOT / SCRIPT_CALCULATE_REINVESTED)],
                check=True,
            )
        except Exception as e:
            return jsonify(
                {"error": f"Correction saved, but recalculation failed: {e}"}
            ), 500
    ...
    def correct_field_value():
        ...
        if not company_symbol or not field_type or new_value is None:
            ...
            return jsonify(
                {"error": "Missing company_symbol, field_type, or new_value"}
            ), 400
        ...
        try:
            ...
            if company_row.empty:
                logger.error(
                    f"Company {company_symbol} with quarter {quarter} not found in flow data"
                )
        ...
        try:
            ...
            for _, row in flow_data.iterrows():
                ...
                if symbol and quarter:
                    ...
                    flow_map[symbol][quarter] = {
                        ...
                        "year": row.get("year", ""),
                        "reinvested_earnings_flow": row.get(
            ...
            for ownership_row in ownership_data:
                ...
                merged_row = {
                    ...
                    f"الأرباح المبقاة للربع السابق ({previous_quarter})": format_value(
                        quarter_data.get("previous_value", "")
                    ),
                    f"الأرباح المبقاة للربع الحالي ({current_quarter})": format_value(
        ...
        try:
            ...
            if ownership_script.exists():
                try:
                    ...
                    logger.info(MSG_OWNERSHIP_UPDATED_OK)
                    return jsonify(
                        {"status": "success", "message": MSG_OWNERSHIP_UPDATED_OK}
                    ), 200
                except subprocess.TimeoutExpired:
        ...
        for quarter_dir in sorted(archives_dir.iterdir()):
            if quarter_dir.is_dir():
                ...
                for file in quarter_dir.glob("financial_analysis_*.xlsx"):
                    ...
                    snapshot_date_str = datetime.fromtimestamp(snapshot_date).strftime(
                        "%Y-%m-%d"
                    )
                    result.append(
                        {
    ...
    def download_snapshot(year_q):
        ...
        if not file_path.exists():
            return json_file_not_found_response()
        return send_file(
            str(file_path),
            as_attachment=True,
            download_name=f"ownership_{year_q}.xlsx",
            mimetype=MIME_XLSX,
        )
    ...
    def list_user_exports():
        ...
        if not user_exports_dir.exists():
            return jsonify([])
        for file in sorted(
            user_exports_dir.glob("financial_analysis_*.xlsx"),
            key=lambda f: f.stat().st_mtime,
            reverse=True,
        ):
            export_date = datetime.fromtimestamp(file.stat().st_mtime).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            result.append(
                {
    ...
    def trigger_quarterly_archive():
        ...
        try:
            ...
            return jsonify(
                {"status": "error", "message": f"Quarterly archiving failed: {str(e)}"}
            ), 500
    ...
    if os.environ.get("WERKZEUG_RUN_MAIN", "true") == "true":
        ...
        logger.info(
            "[Scheduler] 📅 Will run at end of each quarter (Mar 31, Jun 30, Sep 30, Dec 31)"
        )
