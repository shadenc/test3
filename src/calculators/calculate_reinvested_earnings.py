...
def _single_latest_statement_rows(company: str, statements: List[Dict]) -> List[Dict]:
    ...
    if not statements:
        return []
    best = max(statements, key=lambda x: (x["year"], _STMT_TYPE_ORDER.get(x["type"], 999)))
    try:
        v = float(best["value"])
    except (TypeError, ValueError):
        return []
    ...
    formula = (
        f"{t} {y} only — add the prior annual or quarterly PDF to compute quarter-to-quarter flows"
    )
    nan = math.nan
    rows = []
    for q in ("Q1", "Q2", "Q3", "Q4"):
...
def main():
    ...
    if not flow_results:
        ...
        pd.DataFrame(columns=empty_cols).to_csv(FLOW_CSV_PATH, index=False, encoding="utf-8")
        return
