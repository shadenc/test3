...
try:
    from stealth_random import stealth_uniform
except ImportError:
    from .stealth_random import stealth_uniform
...
class QuarterlyUpdateOrchestrator:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initialized orchestrator for {self.current_year} {self.current_quarter}"
        )
    ...
    def _check_existing_ownership_data(self) -> Set[str]:
        ...
        try:
            ...
            existing_symbols = {
                item.get("symbol") for item in data if item.get("symbol")
            }
    ...
    async def update_foreign_ownership_data(self) -> bool:
        ...
        try:
            async with TadawulOwnershipScraper(
    ...
    async def update_net_profit_data(self, symbols: List[str]) -> Dict[str, List[str]]:  # NOSONAR
        ...
        try:
            for i, symbol in enumerate(symbols, 1):
                logger.info(
                    f"📊 Processing net profit for {symbol} ({i}/{len(symbols)})"
                )

                # Check what quarters we already have
                existing_quarters = self._check_existing_net_profit_data(symbol)
                logger.info(
                    f"📁 Existing net profit data for {symbol}: {existing_quarters}"
                )
    ...
    async def run_quarterly_update(self, _force_full_update: bool = False) -> Dict:
        ...
        summary = {
            ...
            "total_new_quarters": sum(
                len(quarters) for quarters in net_profit_results.values()
            ),
        }
